...
st.set_page_config(
    page_title="Artikel Gagal Diproses - AGC Likrea Assistant",
    page_icon=":robot:",
)

page_header(
    title="Daftar Artikel Gagal Diproses",
    description="Berikut adalah daftar artikel yang gagal diproses oleh AGC Likrea. Silakan cek kembali artikel yang gagal diproses untuk memastikan bahwa artikel tersebut dapat diproses oleh AGC Likrea.",
)
supa = SupaListWebHandler()
queue_db = supa.get_article_by_status("error")
queue = {
...
if len(df) == 0:
    st.warning("Tidak ada artikel yang error.")
st.dataframe(
    df,
    hide_index=True,
